...
//////////////////////////////////////////////////////////////////////
// Build a tar file from a list of inputs.
...
var (
	output        = flag.String("output", "", "The output file, mandatory")
	file          multiString
	manifest      = flag.String("manifest", "", "JSON contents of manifest to add to the layer")
	emptyFile     multiString
	emptyDir      multiString
	mode          = flag.String("mode", "", "Force the mode on the added files (in octal).")
	mtime         = flag.String("mtime", "", "Set mtime on tar file entries. May be an integer or the"+" value \"portable\", to get the value 2000-01-01, which is"+" usable with non *nix OSes.")
	emptyRootDir  multiString
	tars          multiString
	debs          multiString
	rpms          multiString
	links         multiString
	directory     = flag.String("directory", "", "Directory in which to store the file inside the layer")
	compression   = flag.String("compression", "", "Compression (`gz` or `bz2`), default is none.")
	modes         multiString
	owners        multiString
	owner         = flag.String("owner", "0.0", "Specify the numeric default owner of all files, e.g., 0.0")
	ownerName     = flag.String("owner_name", "", "Specify the owner name of all files, e.g. root.root.")
	ownerNames    multiString
	rootDirectory = flag.String("root_directory", "./", `Default root directory is named "."`+
		`Windows docker images require this be named "Files" instead of "."`)
	xzPath = flag.String("xz_path", "", `Specify the path to xz as a fallback when the `+
		`Go lzma module is unavailable.`)
)
...
//                name = '/usr/local' + name
//            if strings.HasPrefix(name, './bin'):
//                name = './usr/local' + name[1:]
//            if strings.HasPrefix(name, '/sbin'):
//                name = '/usr/local' + name
//            if strings.HasPrefix(name, './sbin'):
//                name = './usr/local/' + name[1:]
...
//            if strings.HasPrefix(name, './usr/bin'):
//                name = './usr/local' + name[1:]
//            if strings.HasPrefix(name, '/usr/sbin'):
//                name = '/usr/local' + name
//            if strings.HasPrefix(name, './usr/sbin'):
//                name = './usr/local/' + name[1:]
//            if "bin/" in name:
...
func addRpm(tw *tar.Writer, rpmLocation string) {
	p, err := rpm.OpenPackageFile(fn)
	if err != nil {
	...
	for _, file := range p.Files() {
		...
		fmt.Println(file.Name(), name, file.Mode(), mode, size, contents)
		//            with self.write_temp_file(data=f.extractfile(m.name).read()) as temp_file:
		//                from os.path import basename, dirname
		//                self.directory = dirname(name)
		//                name = basename(name)
...
//    symlink = os.path.normpath(symlink)
//    self.tarfile.add_file(symlink, tarfile.SYMTYPE, link=destination)
func addLink(tw *tar.Writer, symlink, destination string) error {
	h := &tar.Header{
...
func main() {
	flag.Var(&file, "file", "A file to add to the layer")
	flag.Var(&emptyFile, "empty_file", "An empty file to add to the layer")
	flag.Var(&emptyDir, "empty_dir", "An empty dir to add to the layer")
	flag.Var(&emptyRootDir, "empty_root_dir", "An empty root directory to add to the layer.  This will create a directory that "+"is a peer of \"root_directory\". \"empty_dir\" creates an empty directory inside of "+"\"root_directory\"")
	flag.Var(&tars, "tar", "A tar to add to the layer")
	flag.Var(&debs, "deb", "A debian package to add to the layer")
	flag.Var(&rpms, "rpm", "A debian package to add to the layer")
	flag.Var(&links, "link", "Add a symlink a inside the layer ponting to b if a:b is specified")
	flag.Var(&modes, "modes", "Specific mode to apply to specific file (from the file argument), e.g., path/to/file=0o455.")
	...
	for _, l := range links {
		if !strings.Contains(l, ":") {
			log.Fatalln("--link value should contains a : separator")
		}
	}
